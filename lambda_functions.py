...
def lambda_handler_primary(event, context):
    ...
    yr = params.get("yr", 75)
    majorId = params.get("majorId", "75/113/to/79/Major/fc50cced-05c2-43c7-7dd5-08dcb87d5deb")

    articulations = getArticulations(fyId, cccId, yr, majorId)

    return {
        'statusCode': 200,
        'body': json.dumps(articulations)
    }
...
def lambda_handler_whitelist(event, context):
    ...
    try:
        ...
        yr = body.get("yr", 75)
        majorId = body.get("majorId", "75/113/to/79/Major/fc50cced-05c2-43c7-7dd5-08dcb87d5deb")
    
    cccCourses = body.get("cccCourses", [])
    artics = body.get("artics", {})

    articulatedCourses = checkArticulations(fyId, cccId, yr, majorId, cccCourses, artics)

    return {
        'statusCode': 200,
        'body': json.dumps(articulatedCourses)
    }
